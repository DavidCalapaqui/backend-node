...
const actulizarImagen = async (req, res = response) =>{
  ...
  if(modelo.img){
    // Hay qe borrar la imagen del servidor
    const pathImagen = path.join(__dirname, '../uploads', coleccion, modelo.img);
    //si la imagen existe en el path
...
const actulizarImagenCloudinary = async (req, res = response) =>{
  ...
  const {tempFilePath} = req.files.archivo; //ruta temporal
  const {secure_url}  = await cloudinary.uploader.upload(tempFilePath);//link de la img en CLOUDINARY
